...
class StreamDeck extends EventEmitter {
  ...
  /**
   * Close the Stream Deck HID device.
   */
  close() {
    this._device.close()
  }
   ...
   * @param {String} kind - only SANS is supported
   ...
   */
  getFontFile(kind, size, color) {
    return Jimp[`FONT_${kind}_${size}_${color}`]
  }

  /**
   * Draw a text string.
   * @param {String} string - the string to draw
   * @param {Integer} buttonNumber - button to draw the string in
   * @param {Object} options alignmentX, alignmentY, font, margin, background
   */
  drawText(string, buttonNumber, options) {
    options = Object.assign({}, {
      alignmentX: Jimp.HORIZONTAL_ALIGN_CENTER,
      alignmentY: Jimp.VERTICAL_ALIGN_MIDDLE,
      fontFile: Jimp.FONT_SANS_16_WHITE,
      margin: 0,
      background: 0x00000000,
      bufferOnly: false
    }, options || {}, { text: string })
    options.x = options.x || options.margin
    options.y = options.y || options.margin
    let hash = JSON.stringify(options)
    let cachedImageBuffer = this._stringCache[hash]
    ...
    return new Promise((resolve, reject) => {
      if (cachedImageBuffer) {
        ...
        Jimp.loadFont(options.fontFile).then(font => {
          new Jimp(ICON_SIZE, ICON_SIZE, options.background, (error, image) => {
            if (error) {
              reject(error)
            } else {
              image.print(
                ...
                options.y,
                options,
                ICON_SIZE - options.x * 2,
                ICON_SIZE - options.y * 2,
                (err, image, { x, y }) => {
                  if (err) {
                    reject(err)
                  } else {
                    ...
                    this._stringCache[hash] = image.bitmap.data
